...
bool Time::isEcoule(tm_t initTime, tm_t deltaTime, tm_t refTime)
{
	...
	else
	{
		...
		else
		{
			...
			{
				if (refTime.tm_ms > addTime.tm_ms)
				{
					ecoule = true;
				}
			}
...
tm_t Time::add(tm_t t1, tm_t t2)
{
	...
	// add millisecondes
	res.tm_ms = t1.tm_ms + t2.tm_ms;
	// verifie overflow ms
	if (res.tm_ms >= TIME_NB_MS)
	{
		res.tm_sec = res.tm_ms / TIME_NB_MS;
		res.tm_ms %= TIME_NB_MS;
	}
...
tm_t Time::tm(uint8_t ms, uint8_t sec, uint8_t min, uint8_t heu)
{
	tm_t t;
	t.tm_ms = ms;
	t.tm_sec = sec;
